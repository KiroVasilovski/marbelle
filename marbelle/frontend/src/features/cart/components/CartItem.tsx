...
const CartItemComponent: React.FC<CartItemProps> = ({ item }) => {
    ...
    return (
        <div className="flex items-center space-x-4 border border-gray-200 rounded p-4">
            <Link to={`/products/${item.product.id}`} className="flex-shrink-0">
                <ProductImage imageUrl={getProductImageUrl(item.product.image)} productName={item.product.name} size="lg" />
            </Link>
